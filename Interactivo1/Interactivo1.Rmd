...
Los vectores son la estructura mas básica de datos en `R`, y es una secuencia de datos, los cuales pueden ser numericos, de carácteres (letras), booleanos (Verdaderos o Falsos), etc. 

La forma mas común de crear un *vector* es con la función `c`, como ejemplo generaremos un vector llamado x, el cual tendra los numero del 0 al 100 en intervalos de 10.
...
En el siguiente ejercicio obten un nuevo vector `y` con el tercer, quinto y décimo elemento:
...
Una de las herramientas mas poderosas para resumir y explorar datos están en el paquete `tidyverse`. Dentor de este paquete usaremos las siguientes funciones:
...
Estas dos funciones usualmente se usan en conjunto, `summarize` resume una variable y `group_by` reune observaciones según una variable. Volviendo al caso de `iris` puedo generar una tabla que sea la media del largo del pétalo usando solo la función `summarize`:
...
A su vez, si quiero agrupar por especie y ver la media de la misma variable ocupo el siguente código
...
El pipeline ` %>%` me ahorra unas pocas lineas. Al usar el pipeline se parte con un data.frame y se agregan funciones de dplyr hasta llegar al resultado deseado. Puedo llegar al mismo resultado que en el codigo anterior con el siguiente código usando el pipeline:
